...
# Adapted regex to handle both `> [!NOTE]` and `>[!NOTE]` (without space) formats
PATTERN = re.compile(
...
def on_page_markdown(markdown, **kwargs):
    def replace_block(match):
        ...
        if title:
            header = f'!!! {mkdocs_type} "{title}"'
        else:
            header = f"!!! {mkdocs_type}"
        ...
        for line in body_lines.split("\n"):
            if line.strip() == "":
                ...
                content = re.sub(r"^" + re.escape(indent) + r"> ?", "", line)
