...
- 昇腾亲和加速算子：提供昇腾亲和多模态的FA、MM、MoE、Quant类算子，以及融合算子，支持通过layer模块对外使用。详情请参见[核心算子加速特性](./features/core_layers.md)。
- 量化稀疏能力：针对昇腾的数据类型和算力分布，提供亲和的算法组合，并通过quantization模块导入使用。详情请参见[量化算法加速特性](./features/quantization.md)和[稀疏算法加速特性](./features/sparse.md)。
- 以存代算：提供DiT Block、Attention等多种粒度的cache算法，以支持不同的视图场景加速。详情请参见[以存代算加速特性](./features/cache.md)。
- 多卡并行：提供CFG、USP等并行能力，融入加速算子的API中，实现接口替换后的自动使能。详情请参见[多卡并行加速特性](./features/parallelism.md)。
- 自动亲和加速：基于torch.compile的inductor机制，自定义融合pass，实现昇腾亲和算子替换。详情请参见[编译优化特性](./features/compilation.md)。
- 动态专家负载均衡：面向DiT-MoE场景，构建了动态专家负载均衡的策略。详情请参见[EPLB加速特性](./features/DyEPLB.md)。
- 显存优化：详情请参见[共享显存](./features/share_memory.md)和[异步cpu offload](./features/cpu_offload.md)。
...
>对于相关但非本仓库聚焦的特性，在examples目录中提供了样例以供参考，例如，服务化部署样例请参考[服务化](https://gitcode.com/Ascend/MindIE-SD/blob/dev/examples/service/service.md)，多模态推理加速样例请参见[Cache](https://gitcode.com/Ascend/MindIE-SD/blob/dev/examples/cache/README.md)。
