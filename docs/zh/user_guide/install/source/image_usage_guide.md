...
执行以下命令启动容器，容器启动命令仅供参考，可根据需求自行修改，命令参数介绍如[表1](#table1)所示。
...
### 3. 安装依赖。
    ...
    使用 MindIE-SD 进行推理前需要安装对应的依赖，依赖安装文件所在路径，以Python 3.11为例：/usr/local/lib/python3.11/site-packages/mindiesd/requirements.txt。使用以下命令安装依赖。

    ```bash
    cd /usr/local/lib/python3.11/site-packages/mindiesd/
