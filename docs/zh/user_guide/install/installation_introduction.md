...
MindIE SD是一个Python包，它可以基于PyTorch构建，独立安装、集成到Python应用程序中。此外，MindIE SD是MindIE软件的高性能视图生成推理组件，安装MindIE软件即可获取MindIE SD。本安装指南将会介绍如何安装MindIE SD，分为MindIE安装和MindIE-SD独立安装两种方式，在MindIE安装中会包含CANN等环境依赖的安装，而MindIE-SD独立安装中不再赘述。
...
### MindIE-SD独立安装
**本文档包含MindIE-SD独立安装的方案**，包含pip源安装或者源码安装两种方式。

### 快速构建开发环境
**本文档包含MindIE-SD快速构建开放环境的方案**，包含使用 Dockerfile 构建开发镜像和使用 Dev Container 构建开发容器两种方式。
