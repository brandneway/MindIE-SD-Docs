...
| 3    | 运行时能力增强。新增多实例共享内存能力，支持多个实例共享权重内存，降低重复占用；新增 layer 级 CPU offload 能力，支持细粒度模块在 CPU 与 NPU 之间动态搬运，缓解显存压力。 |
...
| 2    | FA 量化场景下，FIA 算子输出类型调整为与输入 query 类型保持一致，降低类型不一致带来的兼容性问题。 |
