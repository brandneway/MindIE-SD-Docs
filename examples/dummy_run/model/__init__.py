...
def check_npu():
    ...
    logger.warning("Running %s info -l ...", npu_smi)
...
def resolve_config_path(config_cache, model_id):
    ...
    try:
        return snapshot_download(
            ...
            ignore_patterns=[
                "*.safetensors", "*.bin", "*.msgpack", "*.ckpt", "*.pth",
            ],
            max_workers=1,
        )
    except Exception:
...
class _PhaseTimer:
    ...
    def record_build(self, name, elapsed):
        ...
        if delta > 0:
            logger.warning(
                "  [%s] %.1fs | +%.2fGB | alloc=%.2fGB peak=%.2fGB",
                name, elapsed, delta / (1024 ** 3),
                mem / (1024 ** 3), self._peak / (1024 ** 3),
            )
        else:
            logger.warning(
                "  [%s] %.1fs | alloc=%.2fGB peak=%.2fGB",
                name, elapsed, mem / (1024 ** 3), self._peak / (1024 ** 3),
            )
        self._prev_mem = mem
    ...
    def install(self, pipe):
        for attr in ("text_encoder", "transformer", "transformer_2", "vae"):
            ...
            h_pre = module.register_forward_pre_hook(
                lambda _m, _in, _n=name: self._on_pre(_n)
            )
    ...
    def summary(self):
        ...
        logger.warning("  %-40s %8s %10s", "BUILD", "Time(s)", "Mem(GB)")
        logger.warning(self._DIVIDER_58)
        total_build = 0.0
        for name, elapsed, mem in self._build_records:
            logger.warning("  %-40s %8.1f %10.2f", name, elapsed,
                           mem / (1024 ** 3))
            total_build += elapsed
        logger.warning(self._DIVIDER_58)
        logger.warning("  %-40s %8.1f", "BUILD TOTAL", total_build)

        logger.warning("")
        logger.warning("  %-40s %8s %10s %10s",
        ...
        for name, elapsed, mem in self._infer_warmup:
            logger.warning("  %-40s %8.1f %10.2f %10.2f",
                           name, elapsed, mem / (1024 ** 3),
                           self._peak / (1024 ** 3))
        logger.warning("")
        ...
        for name, elapsed, mem in self._infer_timed:
            ...
            total_timed += elapsed
        logger.warning(self._DIVIDER_68)
        logger.warning("  %-40s %8.1f %10s %10.2f",
                       "OVERALL TOTAL", total_build + total_timed,
                       "", self._peak / (1024 ** 3))
        logger.warning(self._SEP70)
